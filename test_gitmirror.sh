...
function2() {
    ...
    #while true; do
        ...
        if [[ -z "$str" ]]; then
			...
			curl -o $path/update_new.sh https://raw.gitmirror.com/ai-lg/xiaoya/main/update_new.sh
            ...
            bash -c "$(curl https://raw.gitmirror.com/ai-lg/xiaoya/main/update_new.sh)" -s $path $str
...
function6() {
    ...
    if [[ $f6_select == 1 ]]; then
	    ...
	    if [[ $f6_select_3 == 1 ]]; then
	    	...
	    	curl -o $media_dir/rebuild_xy_config.sh https://raw.gitmirror.com/ai-lg/xiaoya/main/rebuild_xy_config.sh
	    	...
	    	if ! [[ $f6_select_5 == [Nn] ]]; then
				...
				read -p "check4:user.sql"
	    		docker start $emby_name
	    		sleep 10
	    		echo -e "${EMBY_URL}"
	    		echo -e "${EMBY_API}"
	    		read -p "check4:emby status,if can in?"
			    ...
			    echo -e $response
			    echo -e $USER_COUNT
			    read -p "check5:response/user_count"
